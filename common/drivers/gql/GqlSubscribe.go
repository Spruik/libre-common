...
func (sc *SubscriptionClient) Subscribe(v string, variables map[string]interface{}, handler func(message *json.RawMessage, err error) error) (string, error) {
	return sc.do(v, variables, handler, "")
}

func (sc *SubscriptionClient) do(v string, variables map[string]interface{}, handler func(message *json.RawMessage, err error) error, name string) (string, error) {
	id := uuid.New().String()
